...
model.1 = function(p = 50, n_train = 50, n_test = 100, pi = 0.5) { 
	...
	train = data.frame(x = train.data$x, y = factor(train.data$y, labels = c("class.1", "class.2")))
	
	test.data = RPModel(1, n_test, p, pi)
	test = data.frame(x = test.data$x, y = factor(test.data$y, labels = c("class.1", "class.2")))
